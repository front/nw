...
if (theme_get_setting('norwegianwood_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('norwegianwood_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'norwegianwood') .'/css/tabs.css', 'theme', 'screen');
}
...
function norwegianwood_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('norwegianwood_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = norwegianwood_id_safe('page-'. $path);
    $body_classes[] = norwegianwood_id_safe('section-'. $section);
...
function norwegianwood_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = norwegianwood_id_safe('node-type-' . $vars['type']);
...
function norwegianwood_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = norwegianwood_id_safe('block-' . $vars['block']->module);
    $classes[] = norwegianwood_id_safe('block-' . $vars['block']->region);
    $classes[] = norwegianwood_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('norwegianwood_block_editing') && user_access('administer blocks')) {
...
function norwegianwood_preprocess_comment(&$vars, $hook) {
...
function norwegianwood_menu_item_link($link) {
...
function norwegianwood_menu_local_tasks() {
  $output = '';
...
function norwegianwood_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = norwegianwood_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function norwegianwood_id_safe($string) {
...
function norwegianwood_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
