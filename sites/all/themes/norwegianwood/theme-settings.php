...
function norwegianwood_settings($saved_settings, $subtheme_defaults = array()) {

  // Get the default values from the .info file.
  $defaults = norwegianwood_theme_get_default_settings('norwegianwood');
  ...
  $form['norwegianwood_block_editing'] = array(
    ...
    '#default_value' => $settings['norwegianwood_block_editing'],
    '#prefix'        => '<strong>' . t('Block Edit Links:') . '</strong>',
  );

  $form['themedev']['norwegianwood_rebuild_registry'] = array(
    '#type'          => 'checkbox',
    '#title'         => t('Rebuild theme registry on every page.'),
    '#default_value' => $settings['norwegianwood_rebuild_registry'],
    '#description'   => t('During theme development, it can be very useful to continuously <a href="!link">rebuild the theme registry</a>. WARNING: this is a huge performance penalty and must be turned off on production websites.', array('!link' => 'http://drupal.org/node/173880#theme-registry')),
    '#prefix'        => '<div id="div-norwegianwood-registry"><strong>' . t('Theme registry:') . '</strong>',
    '#suffix'        => '</div>',
  );
...
function _norwegianwood_theme(&$existing, $type, $theme, $path) {
  ...
  norwegianwood_theme_get_default_settings($theme);
...
function norwegianwood_theme_get_default_settings($theme) {
  $themes = list_themes();
